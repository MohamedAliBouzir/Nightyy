...
export const ThemeContextProvider: FC<IThemeContextProviderProps> = ({ children }) => {
	...
	const [darkModeStatus, setDarkModeStatus] = useState(
		localStorage.getItem('Mig_darkModeStatus')
			? localStorage.getItem('Mig_darkModeStatus') === 'true'
			: process.env.REACT_APP_DARK_MODE === 'true',
	);

	useLayoutEffect(() => {
		localStorage.setItem('Mig_darkModeStatus', darkModeStatus.toString());
	}, [darkModeStatus]);
	...
	const [asideStatus, setAsideStatus] = useState(
		localStorage.getItem('Mig_asideStatus')
			? localStorage.getItem('Mig_asideStatus') === 'true'
			: // @ts-ignore
			  deviceScreen?.width >= process.env.REACT_APP_ASIDE_MINIMIZE_BREAKPOINT_SIZE,
	);
	useLayoutEffect(() => {
		localStorage.setItem('Mig_asideStatus', asideStatus?.toString());
	}, [asideStatus]);
	...
	useLayoutEffect(() => {
		...
		if (deviceScreen?.width >= process.env.REACT_APP_ASIDE_MINIMIZE_BREAKPOINT_SIZE) {
			if (localStorage.getItem('Mig_asideStatus') === 'true') setAsideStatus(true);
